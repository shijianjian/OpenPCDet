...
class CenterHeadKP(CenterHead):
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            pred_keypoints = torch.stack([pred_dict[head_name] for head_name in self.separate_head_cfg.HEAD_ORDER], dim=-1)
            # torch.Size([4, 13, 188, 188, 3])
            pred_center = torch.cat([pred_dict[head_name] for head_name in self.separate_head_cfg.CENTER_ORDER], dim=1)[..., None].permute(0, 4, 2, 3, 1)
            # torch.Size([4, 1, 188, 188, 3])
            pred_keypoints = torch.cat([pred_keypoints, pred_center], dim=1)
            # torch.Size([4, 14, 188, 188, 3])
            # assert False, target_boxes.shape
