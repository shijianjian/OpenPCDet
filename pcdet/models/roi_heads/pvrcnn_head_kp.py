...
class PVRCNNHeadKP(PVRCNNHead):
    ...
    def get_kp_reg_layer_loss(self, forward_ret_dict):
        ...
        kp_mask = forward_ret_dict["keypoint_visibility"].view(-1, code_size // 3)  # (rcnn_batch_size, C)
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            rcnn_kp_loss_reg = self.reg_kp_loss_func(
                rcnn_reg_kp.view(rcnn_batch_size, -1, 3).unsqueeze(dim=0),
                reg_targets.view(rcnn_batch_size, -1, 3).unsqueeze(dim=0),
                mask=kp_mask,
                bboxes=rois_anchor
            )  # [B, M]
    ...
    def build_losses(self, losses_cfg):
        ...
        if model_cfg.LOSS_CONFIG.REG_LOSS_KP == 'smooth-l1':
            self.add_module(
                'reg_kp_loss_func',
                loss_utils.WeightedSmoothL1Loss(code_weights=losses_cfg.LOSS_WEIGHTS['code_weights_kp'])
            )
        elif model_cfg.LOSS_CONFIG.REG_LOSS_KP == 'oks':
            self.add_module(
                'reg_kp_loss_func',
